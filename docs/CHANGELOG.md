...
- 🧠 **Trading philosophy injection completed across legacy + Agent paths** — `GeminiAnalyzer` now resolves skill instructions / default baseline through the same shared skill prompt state as Agent mode, while legacy Analyzer and single-agent prompts no longer hardcode “trend-only / MA5>MA10>MA20” scoring unless that baseline is explicitly injected by the active default skill.
